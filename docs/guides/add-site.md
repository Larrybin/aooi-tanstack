...
这里的 site 不是新的 TanStack route，也不是新的部署分支。一个 site 的最小事实源由三部分组成：
...
- `resources.appStorageBucket` 必须是合法 R2 bucket name。
...
这些命令会通过 `scripts/run-with-site.mjs` 生成当前 site 的 `@/site` 和 content source。不要绕过这个入口直接调用底层 Vite 或 Wrangler 命令。
