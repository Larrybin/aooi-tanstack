...
const requiresClientBuild = process.env.SITE === 'anagram-generator';

test(
  'anagram generator home chunks stay within the 30 kB gzip budget',
  ...
  () => {
    ...
    const chunkPrefixes = ['home.view-', 'anagram-generator-home-'];
    ...
    assert.ok(
      gzipBytes <= 30 * 1024,
      `anagram generator home chunks are ${(gzipBytes / 1024).toFixed(2)} kB gzip (${homeChunkFiles.join(', ')})`
    );
  }
);
