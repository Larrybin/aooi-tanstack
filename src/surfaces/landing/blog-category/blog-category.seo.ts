...
export function getBlogCategorySurfaceHead(
  data: BlogCategoryRouteData | null,
  params?: BlogCategoryHeadParams
): TanStackHead {
  return data?.head ?? getMissingBlogCategoryHead(params);
}

function getMissingBlogCategoryHead(
  params?: BlogCategoryHeadParams
): TanStackHead {
  const locale =
    normalizeLocale(
  ...
  const title = `${slug || 'Category'} | ${messages.metadata.title}`;
