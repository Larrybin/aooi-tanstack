...
export function SettingsPaymentsRouteView({
  data,
}: {
          ...
          <section className="settings-panel">
            ...
            {data.page.records.length === 0 ? (
              ...
              <table>
                <thead>
                  <tr>
                    <th>{data.page.labels.orderNo}</th>
                    <th>{data.page.labels.productName}</th>
                    <th>{data.page.labels.status}</th>
                    <th>{data.page.labels.type}</th>
                    <th>{data.page.labels.price}</th>
                    <th>{data.page.labels.paidAmount}</th>
                    <th>{data.page.labels.discountAmount}</th>
                    <th>{data.page.labels.createdAt}</th>
                    <th>{data.page.labels.invoice}</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={record.id}>
                      <td>
                        ...
                        {record.orderNo ? (
                          <button
                            type="button"
                            onClick={() => copyOrderNo(record.orderNo)}
                          >
                      ...
                      <td>
                        {record.invoiceHref ? (
                          <a
                            href={record.invoiceHref}
                            target={
                              record.invoiceExternal ? '_blank' : undefined
                            }
                            rel={
                              record.invoiceExternal
                                ? 'noopener noreferrer'
                                : undefined
                            }
                          >
                            {data.page.labels.invoice}
                          </a>
                        ) : (
                          '-'
                        )}
