...
test('architecture: settings-store does not depend on process-local settings cache state', async () => {
  const files = await readSourceFiles();
  ...
  assert.equal(
    ...
    false,
    'settings-store.ts 不应依赖 process-local settings cache'
  );
  assert.equal(
    files.some(
      ({ repoPath }) =>
        ...
        repoPath ===
          'src/domains/settings/application/settings-cache-version.ts'
    ),
    false,
    'settings domain 不应保留 isolate-local cache helper/version 文件'
  );
});
