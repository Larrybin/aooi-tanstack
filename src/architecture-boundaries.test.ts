...
test('architecture: settings-store owns settings cache invalidation boundary', async () => {
  const files = await readSourceFiles();
  const settingsStore = files.find(
    ({ repoPath }) =>
      repoPath === 'src/domains/settings/application/settings-store.ts'
  );
  const settingsCache = files.find(
    ({ repoPath }) =>
      repoPath === 'src/domains/settings/application/settings-cache.ts'
  );

  assert.ok(settingsStore, 'settings-store.ts 应存在');
  assert.ok(settingsCache, 'settings-cache.ts 应存在');

  assert.equal(
    readImportSpecifiers(settingsStore.content).includes('./settings-cache'),
    true,
    'settings-store.ts 应通过 settings-cache 拥有 cache invalidation boundary'
  );
  assert.equal(
    /revalidateSettingsCacheTag\s*\(\s*CONFIGS_CACHE_TAG\s*\)/.test(
  ...
  assert.equal(
    /revalidateSettingsCacheTag\s*\(\s*PUBLIC_CONFIGS_CACHE_TAG\s*\)/.test(
      settingsStore.content
    ),
    true,
  ...
  assert.equal(
    readImportSpecifiers(settingsCache.content).includes('next/cache'),
    false,
    'settings-cache.ts 不应依赖 next/cache'
  );
});
