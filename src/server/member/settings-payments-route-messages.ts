...
export type SettingsPaymentsRouteMessages = {
  payments: Messages;
  sidebar: Messages;
};

export async function loadSettingsPaymentsRouteMessages(
  locale: string
): Promise<SettingsPaymentsRouteMessages | null> {
  const [basePayments, baseSidebar] = await Promise.all([
    importMessages('settings/payments', baseMessagesLocale),
  ...
  const [localizedPayments, localizedSidebar] = await Promise.all([
    importMessagesOptional('settings/payments', locale),
    importMessagesOptional('settings/sidebar', locale),
  ]);

  return {
    payments: localizedPayments
      ? (mergeDeep(basePayments, localizedPayments) as Messages)
      : basePayments,
