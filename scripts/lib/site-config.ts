...
import { validateSiteConfig, type SiteConfig } from '../site-schema.ts';

export const TEST_SITE_KEY = 'dev-local';

type SitePathOptions = { rootDir?: string; siteKey?: string };

export function resolveRequiredSiteKey(
...
}: SitePathOptions = {}): string {
  return path.resolve(rootDir, 'sites', siteKey, 'site.config.json');
}

export function listConfiguredSiteKeys(rootDir = process.cwd()): string[] {
...
function formatConfiguredSiteKeys(siteKeys: readonly string[]): string {
...
export function readCurrentSiteConfig({
  ...
  const site: unknown = JSON.parse(raw);
